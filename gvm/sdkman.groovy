...
new SdkmanWindowsFixer().fixEnvironment()

/**
 * When using Sdkman  under Cygwin, Sdkman only sets the Cygwin environment vars correctly.
 * This script sets the Windows environment variables for all installed modules.
 */
class SdkmanWindowsFixer
{
    ...
    /**
     * Lists all the Sdkman moduls. These are the subdirectories in the candidate subdir.
     */
    private File[] getModuleList()
    {
        // find proper installation directory
        def sdkmanRoot = new File("$homeDir\\.sdkman\\candidates")

        // iterate over all package directories
        def result = []
        sdkmanRoot.eachDir { result << it }

        // return them as module list
        result
    }
    ...
    /**
     * Sets a Windows environment variable to all available Sdkman modules.
     */
    void fixEnvironment()
    {
        // read path value
        ...
        // read available candidates
        def availableCandidates = new File("$homeDir\\.sdkman\\var\\candidates")
                .readLines()
                .get(0)
                .split(',')

        availableCandidates.each { candiate ->
            def current = "$homeDir\\.sdkman\\candidates\\$candiate\\current"
            def envVar = "${candiate.toUpperCase()}_HOME"
    ...
    /**
     * Reads an user specific environment variable from the registry.
     */
